...
### **Software Engineer** • **Backend & Distributed Systems** • **Cloud-Native Platforms**

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=500&size=22&pause=1000&color=3584E4&center=true&vCenter=true&random=false&width=700&lines=Building+scalable+cloud-native+backends;Java+%7C+Python+%7C+Go+%7C+Microservices;Identity+%26+Trust+Platform+%40+Ascensus;AWS+%7C+Kubernetes+%7C+Kafka+%7C+Docker;Open+to+interesting+problems" alt="Typing SVG" />

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/kalgutkarsahil)
...
- The API is authenticated with **scoped bearer tokens**, and the server stores only their SHA-256 digests. Plain SHA-256 rather than bcrypt, deliberately: password hashes are slow to defeat brute force over a small guessable space, and a 256-bit random token has no such space, so a slow hash buys nothing and costs every request. It is timing-safe because the guess is hashed *before* the lookup — I wrote a constant-time compare after it, then removed it, because it compared a value against itself
- Startup **fails closed**: no tokens and no explicit opt-out is refused, because a server that comes up, serves, and looks healthy with an open control plane is the worst kind of silent failure. Adding auth introduced a bug the smoke run caught — audit logging keyed on the HTTP method, and scoring is a POST, so every prediction was logged as a change
- A **dashboard** at `/` — server-rendered HTML with **zero JavaScript**, which is the decision rather than a shortcut: `html/template` contextual escaping is the XSS defence, and no JS is what lets the CSP be `default-src 'none'` with no `script-src`, so "we escaped it correctly" becomes something the browser enforces. Deploys, canaries and rollbacks are two-step — plan, then apply — and apply refuses if somebody else changed the policy while you were looking at the confirmation. A browser gets a sign-in redirect where an API client gets a 401
- Browsers can sign in and hold a **session cookie** — opaque id rather than a signed identity, so logout can actually revoke it; `HttpOnly`/`Secure`/`SameSite`/no-`Domain` each closing a named hole; and CSRF double-submit as a second lock on the door SameSite already closes, because SameSite is a *browser* behaviour and a token the attacker cannot read is not. A session is never a weaker front door: the token is verified through the same path an API call takes, and a bearer token beats an ambient cookie
- Sessions **renew themselves** and `logout` **actually revokes** at the provider. The subtlety worth naming is concurrency: two commands at once both refresh, and against a rotating provider the second invalidates the first — so refresh runs under a file lock that re-reads inside it, and eight concurrent workers produce exactly one refresh. Verified revocation by restoring the deleted credential and watching the provider refuse it
- `modelforgectl login` signs a person in through the **RFC 8252 native-app flow** — public client with **PKCE** instead of a secret, `state` and `nonce`, redirect to `127.0.0.1` rather than `localhost` so a hostile resolver cannot capture the code, credential stored 0600. Smoke-testing it found a bug I would not have predicted: leaving the OAuth client-auth style unset makes x/oauth2 probe by retrying, and since an authorization code is single-use the probe *burns* it — so the error you see points at the code rather than at the real failure
- People authenticate with **OIDC**, so the audit log names a person rather than a shared credential — the readable identity *and* the stable subject, because emails change and subjects do not. Service tokens and user logins coexist, told apart by shape. I deliberately did **not** write the JWT verification: a bug in the scorer is a wrong number a differential test catches, a bug in JWT verification is a total auth bypass where everything looks fine while anybody forges an admin identity. The tests prove the wiring — real JWKS over HTTP, and forged signatures, `alg:none`, swapped claims and wrong audiences each refused
- Tokens **rotate without a restart** — read from a file re-read on SIGHUP and on a timer, swapped through an atomic pointer so the read path takes no lock and a request sees exactly one generation of the set. 400 requests across 40 live reloads, all 200, same PID. A malformed file leaves the running set in force rather than locking everyone out, and entries can carry an expiry enforced per request
- Repeated auth failures are **throttled**, and scoping that claim honestly was the work: it is not brute-force protection against 256-bit tokens, it bounds the *cost* of failures — 200 bad-token requests now produce 6 log lines instead of 200. I first checked the limit before authenticating; the test showed that refuses a correct credential when an address is shared behind a NAT, so authentication runs first and a valid token is never throttled. The tracking table fails open when saturated, so an attacker cannot fill it to lock everyone out
- Every command and number in the README was run against a live server before being written down, including a `known limitations` section — 89.9% coverage, and the suite fails rather than skips when Postgres is missing
- `Go · XGBoost · Model Serving · Canary Deployments · Shadow Traffic · Drift Detection (PSI) · Scoped Token Auth · Postgres · Prometheus`
...
**M.S. Computer Science** — University of Massachusetts, Boston (2022 – 2024)
**B.E. Computer Engineering** — University of Mumbai (2017 – 2021)
